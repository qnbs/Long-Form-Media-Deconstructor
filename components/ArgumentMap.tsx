...
const ArgumentNode: React.FC<ArgumentNodeProps> = ({ argument, onHighlight }) => {
  ...
  return (
    <li className="pl-4 border-l-4 border-brand-primary/30">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full text-left font-semibold text-slate-800 dark:text-slate-100 mb-3 flex items-center gap-2 hover:text-brand-primary transition-colors"
                  ...
                  <ul className="list-disc list-inside space-y-1 text-slate-700 dark:text-slate-300">
                    {argument.supportingEvidence.map((evidence, i) => (
                      <li 
                        ...
                        className="p-1 -ml-1 rounded cursor-pointer hover:bg-brand-primary/20 transition-all duration-200"
...
export const InteractiveArgumentMap: React.FC<InteractiveArgumentMapProps> = ({ argumentMap, onHighlight }) => {
  return (
    <div className="bg-white/10 dark:bg-slate-900/20 backdrop-blur-xl border border-white/20 dark:border-slate-700/50 shadow-lg rounded-xl p-6 space-y-6 ring-1 ring-inset ring-white/10 dark:ring-slate-700/50">
      <div>
        <h3 className="text-lg font-semibold text-brand-primary mb-2">Main Thesis</h3>
        <p className="text-slate-700 dark:text-slate-300 pl-4 border-l-4 border-brand-primary/30">{argumentMap.mainThesis}</p>
      </div>
      <div>
        <h3 className="text-lg font-semibold text-brand-primary mb-4">Primary Arguments</h3>
