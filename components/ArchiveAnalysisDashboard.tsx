...
const TabButton: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; }> = ({ icon, label, isActive, onClick }) => (
    <button onClick={onClick} className={`flex items-center gap-3 px-4 py-2 rounded-t-lg border-b-2 transition-colors duration-300 ${isActive ? 'border-brand-primary text-brand-primary bg-slate-200 dark:bg-slate-800' : 'border-transparent text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'}`}>
        {icon}
        ...
        switch (activeTab) {
                ...
                return (
                    <div className="flex flex-col h-full">
                        <div className="p-4 border-b border-slate-300 dark:border-slate-700">
                             <input type="text" placeholder="Search timeline..." value={timelineSearch} onChange={e => setTimelineSearch(e.target.value)} className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-primary"/>
                        </div>
                            ...
                            {filteredTimeline.map((item, index) => (
                                ...
                                >
                                    <div className="flex flex-col items-center h-full">
                                        ...
                                        <div className="w-0.5 grow bg-slate-300 dark:bg-slate-700"></div>
                                    </div>
                                    <div className="pb-8 flex-1">
                                        <p className="font-bold text-slate-800 dark:text-slate-200">{item.date}: <span className="font-semibold text-sky-600 dark:text-sky-300">{item.event}</span></p>
            ...
            case 'entities':
                ...
                return (
                     <div className="flex flex-col h-full">
                        <div className="p-4 border-b border-slate-300 dark:border-slate-700">
                             <input type="text" placeholder="Search entities..." value={entitySearch} onChange={e => setEntitySearch(e.target.value)} className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-primary"/>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-4 flex-grow overflow-hidden">
                            <div className="md:col-span-1 h-full overflow-y-auto pr-2">
                                <ul className="space-y-2">
                                    {filteredEntities.map((entity, index) => (
                                        <li key={index}>
                                            <button onClick={() => setSelectedEntity(entity)} className={`w-full text-left p-3 rounded-lg transition-colors ${selectedEntity?.name === entity.name ? 'bg-brand-primary text-white' : 'bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700'}`}>
                            ...
                            <div className="md:col-span-2 bg-slate-200/50 dark:bg-slate-800/50 p-4 rounded-lg h-full overflow-y-auto">
                                {selectedEntity ? (
                                    <div>
                                        ...
                                        <ul className="space-y-4 pr-2">
                                            {selectedEntity.mentions.map((mention, i) => (
                                                <li 
                                                    key={i} 
                                                    className="border-l-4 border-slate-400 dark:border-slate-600 pl-4"
                ...
                return (
                    <div className="space-y-6 h-full overflow-y-auto p-4">
                        {result.connections.map((conn, index) => (
                            <div key={index} className="bg-slate-200 dark:bg-slate-800 p-4 rounded-lg">
                                ...
                                <div className="mt-3 space-y-2">
                                    {conn.connectingDocuments.map((doc, i) => (
                                        <div 
                                            key={i} 
                                            className="border-l-4 border-slate-400 dark:border-slate-600 pl-4 text-sm"
        ...
        <div className="w-full animate-fade-in max-w-7xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mt-8">
                <main className="lg:col-span-4">
                    ...
                    <div className="bg-slate-200/30 dark:bg-slate-800/30 rounded-b-lg min-h-[70vh] w-full h-[70vh] overflow-hidden">
                        {renderTabContent()}
                    </div>
                </main>
                <aside className="lg:col-span-1 flex flex-col">
                     <div className="flex-shrink-0">
                         <div className="flex p-1 bg-slate-200 dark:bg-slate-800 rounded-lg mb-2">
                            <button onClick={() => setSidebarView('notes')} className={`w-1/2 flex items-center justify-center gap-2 text-sm p-2 rounded-md transition-colors ${sidebarView === 'notes' ? 'bg-brand-secondary text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-300 dark:hover:bg-slate-700'}`}>
                                <NotesIcon /> Notes
                            </button>
                            <button onClick={() => setSidebarView('chat')} className={`w-1/2 flex items-center justify-center gap-2 text-sm p-2 rounded-md transition-colors ${sidebarView === 'chat' ? 'bg-brand-secondary text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-300 dark:hover:bg-slate-700'}`}>
                                <ChatIcon /> Chat
                            </button>
                         </div>
                    </div>
                    ...
                    <div className="flex-grow min-h-0">
                        {sidebarView === 'notes' ? (
                            <div className="h-full flex flex-col gap-4">
                                <textarea value={notes} onChange={handleNotesChange} placeholder="Add your notes and observations here... Your notes will be saved in this browser." className="w-full flex-grow bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary"></textarea>
                                <div>
